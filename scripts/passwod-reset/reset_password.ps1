...
while(($eingabe = Read-Host -Prompt "Wählen Sie bitte eine Raumnummer -  z.B 4: Bei exit Q") -ne "Q"){
	...
	Set-ADAccountPassword -Identity "s$zahlEins.p$zahlZwei" -Reset -NewPassword (ConvertTo-SecureString -AsPlainText "Changeme1!" -Force)  | Set-ADuser -ChangePasswordAtLogon $True
